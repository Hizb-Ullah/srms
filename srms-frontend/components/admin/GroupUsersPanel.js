...
export default function GroupUsersPanel({ group, subRoles, title }) {
  ...
  const [form, setForm]           = useState({ name: '', email: '', password: '', subRole: '' })
  ...
  const openCreate = () => {
    setForm({ name: '', email: '', password: '', subRole: subRoles[0] || '' })
    setShowPw(false)
    setCreateOpen(true)
  }
  ...
  const handleCreate = async (e) => {
    ...
    setSaving(true)
    try {
      await createUser({ ...form, role: 'surveyor', group })
  ...
  const openEdit = (u) => {
    setSelected(u)
    setForm({ name: u.name, email: u.email, password: '', subRole: u.subRole || subRoles[0] || '' })
    setEditOpen(true)
  }
  ...
  const handleEdit = async (e) => {
    ...
    setSaving(true)
    try {
      await updateUser(selected._id, { name: form.name, email: form.email, group, subRole: form.subRole })
    ...
    <DashboardLayout title={title}>
      <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-6">
        ...
        {loading ? <TableSkeleton rows={4} /> : users.length === 0 ? (
          <p className="text-slate-500 text-sm">No {group} users yet.</p>
        ) : (
                ...
                <tr key={u._id} className="border-b border-slate-50 hover:bg-slate-50 transition">
                  ...
                  <td className="py-3">
                    <div className="flex items-center gap-1.5">
                      <button onClick={() => openEdit(u)} className="p-1.5 rounded-lg text-slate-500 hover:bg-slate-100 hover:text-indigo-600 transition" title="Edit">
                        <Pencil size={15} />
                      </button>
                      <button onClick={() => { setSelected(u); setResetPw(''); setShowPw(false); setResetOpen(true) }} className="p-1.5 rounded-lg text-slate-500 hover:bg-slate-100 hover:text-amber-600 transition" title="Reset password">
                        <KeyRound size={15} />
                      </button>
                      <button
                        onClick={() => handleToggleLock(u._id)}
                        className={u.isLocked
                      ...
                      <button onClick={() => { setSelected(u); setDeleteOpen(true) }} className="p-1.5 rounded-lg text-slate-500 hover:bg-rose-50 hover:text-rose-600 transition" title="Delete">
                        <Trash2 size={15} />
                      </button>
                    </div>
                  </td>
      ...
      {/* Create */}
      <Modal open={createOpen} onClose={() => setCreateOpen(false)} title={`Add ${group} user`}>
        <form onSubmit={handleCreate} className="space-y-4">
          ...
          <div>
            ...
            <div className="relative">
              <input type={showPw ? 'text' : 'password'} required minLength={8} value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} className={`${inputCls} pr-10`} />
          ...
          <div>
            ...
            <select value={form.subRole} onChange={(e) => setForm({ ...form, subRole: e.target.value })} className={inputCls}>
      ...
      {/* Edit */}
      <Modal open={editOpen} onClose={() => setEditOpen(false)} title={`Edit ${group} user`}>
      ...
      <Modal open={resetOpen} onClose={() => setResetOpen(false)} title={`Reset password — ${selected?.name || ''}`}>
        <form onSubmit={handleReset} className="space-y-4">
          <div>
            ...
            <div className="relative">
              <input type={showPw ? 'text' : 'password'} required minLength={8} value={resetPw} onChange={(e) => setResetPw(e.target.value)} placeholder="Min 8 characters" className={`${inputCls} pr-10`} />
              <button type="button" onClick={() => setShowPw(!showPw)} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600">
                {showPw ? <EyeOff size={16} /> : <Eye size={16} />}
              </button>
            </div>
