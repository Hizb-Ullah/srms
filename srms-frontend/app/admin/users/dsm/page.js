...
export default function DsmUsersPage() {
  ...
  const [form, setForm]             = useState({ name: '', email: '', password: '', subRole: SUB_ROLES[0] })
  ...
  const handleCreate = async (e) => {
    ...
    try {
      await createUser({ name: form.name, email: form.email, password: form.password, role: 'officer', group: GROUP, subRole: form.subRole })
  ...
  return (
    <DashboardLayout title="DSM Users">
      <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="font-semibold text-slate-800">DSM Users</h3>
          ...
          <button onClick={() => { setForm({ name: '', email: '', password: '', subRole: SUB_ROLES[0] }); setShowPw(false); setCreateOpen(true) }}
      ...
      <Modal open={createOpen} onClose={() => setCreateOpen(false)} title="Add DSM user">
