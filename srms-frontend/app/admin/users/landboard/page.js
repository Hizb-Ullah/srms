...
export default function LandBoardUsersPage() {
  ...
  return (
    <DashboardLayout title="Land Board Users">
      <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="font-semibold text-slate-800">Land Board Users</h3>
          ...
          <button onClick={() => { setForm({ name: '', email: '', password: '', subRole: SUB_ROLES[0], surveyorCode: '' }); setShowPw(false); setCreateOpen(true) }}
            ...
            <Plus size={16} /> Add Land Board user
          </button>
        </div>

        {loading ? <TableSkeleton rows={4} /> : users.length === 0 ? (
          <p className="text-slate-500 text-sm">No Land Board users yet.</p>
        ) : (
      ...
      <Modal open={createOpen} onClose={() => setCreateOpen(false)} title="Add Land Board user">
        <form onSubmit={handleCreate} className="space-y-4">
          ...
          <div><label className="block text-sm font-medium text-slate-600 mb-1">Surveyor Code #</label>
            <input type="text" value={form.surveyorCode} onChange={e => setForm({...form, surveyorCode: e.target.value})} placeholder="e.g. LB/001" className={cls} /></div>
      ...
      <Modal open={editOpen} onClose={() => setEditOpen(false)} title="Edit Land Board user">
